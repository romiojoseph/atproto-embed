...
function buildWidget(name) {
  const SRC_JS = path.join(__dirname, "..", "src", name + ".js");
  const SRC_CSS = path.join(__dirname, "..", "src", name + ".css");
  const OUT_JS = path.join(DIST_DIR, name + ".js");
  const OUT_CSS = path.join(DIST_DIR, name + ".css");
...
// Match the entire Icons section: the comment, getBasePath(), and getIconUrl()
  ...
  if (!iconsRegex.test(js)) {
    ...
    console.error("  Make sure src/" + name + ".js has the /* ───── Icons ───── */ comment block");
...
// Minify CSS
  ...
  if (!injectRegex.test(js)) {
    ...
    console.error("  Make sure src/" + name + ".js has the injectStyles(root) function");
...
// Quick sanity checks on the output
  ...
  console.log(`✓ ${name}.js — SVGs inlined: ${svgFiles.length} icons`);
  console.log(`✓ ${name}.js — CSS inlined (${(Buffer.byteLength(cssMinified, "utf8") / 1024).toFixed(1)} KB)`);
  console.log(`✓ dist/${name}.js written — ${(outSize / 1024).toFixed(1)} KB (src was ${(srcSize / 1024).toFixed(1)} KB)`);
  console.log(`✓ dist/${name}.css written (standalone copy)`);
  console.log("");
}
